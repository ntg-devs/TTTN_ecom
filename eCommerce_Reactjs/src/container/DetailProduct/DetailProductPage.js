...
import { trackKOLLinkAccess } from "../../services/affiliateService";
...
function DetailProductPage(props) {
  ...
  const handleAccessKOLLink = async () => {
    try {
      const currentUrl = window.location.href;
      console.log("Tracking KOL link:", currentUrl);

      const res = await trackKOLLinkAccess({ url: currentUrl });
      ...
      if (res?.data?.errCode === 0) {
        ...
        console.warn(
          "Failed to track:",
          res?.data?.errMessage || "Unknown error"
        );
      }
    } catch (error) {
      console.error("Error while tracking KOL link:", error.message);
    }
  };
