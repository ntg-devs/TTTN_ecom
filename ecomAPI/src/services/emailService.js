...
let sendSimpleEmail = async (dataSend) => {
    ...
    if (dataSend.type === 'verifyEmail') {
        let info = await transporter.sendMail({
            from: '"CLOTHSHOP 👻" <giangnguyen6777@gmail.com>', // sender address
    ...
    if (dataSend.type === 'kolStatus') {
        let info = await transporter.sendMail({
            from: '"CLOTHSHOP 👻" <giangnguyen6777@gmail.com>', // sender address
            to: dataSend.email, // list of receivers
            subject: dataSend.subject || "KOL Application Status | CLOTHSHOP", // Subject line
            html: dataSend.htmlContent // Use the provided HTML content
        });
    }
}
...
let getBodyHTMLEmailVerify = (dataSend) => {
    let firstName = dataSend.firstName ?? '';
    let lastName = dataSend.lastName ?? '';
...
//             let info = await transporter.sendMail({
//                 from: '"ADMIN 👻" <giangnguyen6777@gmail.com>', // sender address
//                 to: dataSend.email, // list of receivers
