            ...
            if (!data.id || !data.limit || !data.offset) {
                ...
                if (productdetail.rows && productdetail.rows.length > 0) {
                    for (let i = 0; i < productdetail.rows.length; i++) {
                        productdetail.rows[i].productImages = await db.ProductImage.findAll({
                            where: { productdetailId: productdetail.rows[i].id }
                        })
                        productdetail.rows[i].productsize = await db.ProductDetailSize.findAll({
                            where: { productdetailId: productdetail.rows[i].id }
                        })
                        if (productdetail.rows[i].productImages && productdetail.rows[i].productImages.length > 0) {
                            for (let j = 0; j < productdetail.rows[i].productImages.length > 0; j++) {
                                productdetail.rows[i].productImages[j].image = new Buffer(productdetail.rows[i].productImages[j].image, 'base64').toString('binary')
                            }
                        }
