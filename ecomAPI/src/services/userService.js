  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (data.roleId === "employee") {
        ...
        throw new Error(
          'roleId không hợp lệ (chỉ nhận "employee" hoặc "customer")'
        );
      }
...
let handleLogin = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Check password
      const check = bcrypt.compareSync(data.password, account.password);
      if (!check) {
        return resolve({
          errCode: 3,
      ...
      userData.accessToken = CommonUtils.encodeToken(account.accountId);

      // Lấy role
      const roles = await db.AccountRole.findAll({
      ...
      for (let i = 0; i < roles.length; i++) {
        ...
        if (userData.user) {
          userData.user.accountId = account.accountId;
          userData.user.roleId = roleId;
          break; // ✅ tránh bị ghi đè khi có nhiều role
        }
      }
